    ...
    def __init__(self, opt):
        ...
        if 'gt_path' in opt:
            if isinstance(opt['gt_path'], str):
                ...
                #self.paths.extend(opt['gt_path'])
                #self.paths.extend(sorted([str(x) for x in Path(opt['gt_path'][0]).glob('*.'+opt['image_type'])]))
                if len(opt['gt_path']) > 1:
                    for i in range(len(opt['gt_path'])):
                        self.paths.extend(sorted([str(x) for x in Path(opt['gt_path'][i]).glob('*.'+opt['image_type'])]))
                    self.paths = opt['gt_path']
